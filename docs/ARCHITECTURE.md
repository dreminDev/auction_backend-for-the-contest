...
Аукцион создается со статусом `active`, но **таймер раунда не запускается**. Поля `roundStartTime` и `roundEndTime` устанавливаются в `null` до первой ставки.

### Отложенный старт раунда

Таймер раунда запускается только после первой ставки:
1. Аукцион создаётся с `roundStartTime = null` и `roundEndTime = null`
2. При первой ставке в раунде устанавливается `roundStartTime = now` и `roundEndTime = now + roundDuration`
3. Воркер пропускает аукционы с `roundEndTime = null` (ожидание первой ставки)
4. При переходе в следующий раунд без перенесённых ставок таймер снова сбрасывается в `null`

Это позволяет участникам не торопиться - раунд не закончится, пока никто не сделал ставку.
...
#### Продление раунда (антиснайпинг система)

Если ставка размещается в последние 10 секунд до окончания раунда, раунд автоматически продлевается на 10 секунд от текущего момента. Это предотвращает снайпинг (ставки в последний момент).
...
2. **Логирование победителей**: В таблицу `Action` записывается информация о победителях с типом `win`:
   - `userId` - ID пользователя
   - `auctionId` - ID аукциона
   - `round` - номер раунда
   - `amount` - сумма ставки
   - `place` - занятое место (1, 2, 3...)
   - `giftCollectionId` - ID коллекции подарка
3. **Удаление ставок**: Выигравшие ставки удаляются из базы данных
4. **Важно**: Пользователи, выигравшие в текущем раунде, могут делать новые ставки в следующих раундах, но их выигравшая ставка не переносится

Историю победителей можно получить через эндпоинт `GET /auction/winners?auctionId=...`

#### Обработка проигравших ставок (losers)

**Если есть следующий раунд и есть проигравшие ставки:**
1. Ставки переводятся в следующий раунд (обновляется поле `round`)
2. Аукцион переходит к следующему раунду
3. Таймер запускается сразу (т.к. уже есть перенесённые ставки)

**Если есть следующий раунд, но нет проигравших ставок:**
1. Аукцион переходит к следующему раунду
2. Таймер сбрасывается (`roundStartTime = null`, `roundEndTime = null`)
3. Ожидание первой ставки для запуска таймера
...
Все операции логируются в таблицу `Action`:

| Тип | Описание | Метаданные |
|-----|----------|------------|
| `register` | Регистрация пользователя | — |
| `balance` | Пополнение баланса | `amount`, `type` |
| `bet` | Размещение ставки | `auctionId`, `amount`, `balanceType`, `oldBalance`, `newBalance` |
| `win` | Победа в раунде | `auctionId`, `round`, `amount`, `place`, `giftCollectionId` |
